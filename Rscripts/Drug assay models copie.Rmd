...
sorted_data <- subset[order(subset$dose), ]
# Calculate the medians of 'value' at each dose
medians <- tapply(sorted_data$value, sorted_data$dose, FUN = median)
# Generate a sequence of intermediate doses
intermediate_dose <- seq(min(subset$dose), max(subset$dose), length.out = 100)
# Interpolate the medians at the intermediate doses
interpolated_values <- approx(x = names(medians), y = medians, xout = intermediate_dose)$y
...
combined_data <- rbind(select(sorted_data,-ID), interpolated_data)
# Sort the combined data by dose
combined_data <- combined_data[order(combined_data$dose), ]
