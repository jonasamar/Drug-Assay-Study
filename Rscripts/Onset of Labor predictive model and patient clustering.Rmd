...
xxx<-function(X, Y, foldid, i, parm, predict.simulated.data=FALSE)
{
  ...
  if(parm$scale=='Patient')
  {
    ...
    # print('------------')
    # print(patient.col.means[["Tregs_STAT6_IL246"]])
    # print(length(patient.col.means))
    # print(class(patient.col.means))
    # print('------------')
  ...
  if (predict.simulated.data){
    ...
    # print('------------')
    # print(means[["Tregs_STAT6_IL246"]])
    # print(length(means))
    # print('------------')
    sds = colSds(unscaled.X)
    # print('------------')
    # print(sds[["Tregs_STAT6_IL246"]])
    # print(length(sds))
    # print('------------')
    ...
    median_prepro_data <- dplyr::select(ret$median.simulated.TTL, -c(drug, sampleID, centroid, dose)) %>%
        ...
        dplyr::select(intersect(colnames(CYTOF),colnames(ret$median.simulated.TTL)), -names(ret$rm_cols)) %>%
        ...
        mutate(across(everything(), ~ (. - patient.col.means[[cur_column()]] - means[[cur_column()]])/sds[[cur_column()]])) %>%
    ...
    individual_prepro_data <- dplyr::select(ret$individual.simulated.TTL, -c(drug, sampleID, centroid, dose)) %>%
        # Removing the columns which were removed in CYTOF
        dplyr::select(intersect(colnames(CYTOF),colnames(ret$individual.simulated.TTL)), -names(ret$rm_cols)) %>%
        # Imputing the missing values with the medians of the corresponding CYTOF columns
        mutate(across(everything(), ~ ifelse(is.na(.), median(CYTOF[[cur_column()]], na.rm = TRUE), .))) %>%
        # Applying the patient and gobal scaling applyed to the training data
        mutate(across(everything(), ~ (. - patient.col.means[[cur_column()]] - means[[cur_column()]])/sds[[cur_column()]])) %>%
...
ret <- xxx(data.matrix(CYTOF), Yh, Id, 10, parm, TRUE)
```
...
```{r}
######################### WARNING : THIS CELL TAKES 20-30min TO RUN ! #########################
...
registerDoSNOW(cl)
# Getting models and predictions
prdC=foreach(i=seq(npt),.packages = c("dplyr", "tibble")) %dopar% xxx(data.matrix(CYTOF_EGA), Yh, Id, i, parm, TRUE)
list_preds<-aggregation.of.predictions(prdC)
# Closing backend configuration
stopCluster(cl)
```
...
```{r}
# Model chosen
model.name = "Random.Forest" # ElasticNet or Ridge.Regression or Lasso.1se or Adaptive.Lasso or Random.Forest
# list_preds
load(paste0(out_path, "/best ", model.name," predictions.rda"))
# prdC
...
for (i in 1:length(prdC)){
  all.simulated.predictions <- rbind(all.simulated.predictions, 
                                     prdC[[i]]$median.simulated.TTL %>% rename("TTL.prediction"="median_predictions"), 
                                     prdC[[i]]$individual.simulated.TTL %>% rename("TTL.prediction"="individual_predictions"))
}
```
...
for (centroid.val in unique(all.simulated.predictions$centroid)){
  ...
  for (c_k in 1:4){
    plot_delay_distribution(c_k, centroid.val, 25, 40)
  }
  dev.off()
}
```
...
plot_delay_distribution_across_clusters(target_drug, c_k, 30, 40)
dev.off()
```
