...
Visualization of the performances of the models.
...
```{r}
# Model chosen
model.name = "ElasticNet" # ElasticNet or Ridge.Regression or Lasso.1se or Adaptive.Lasso
# list_preds
load(paste0(out_path, "/best ", model.name," predictions.rda"))
# prdC
...
for (i in 1:length(prdC)){
  all.simulated.predictions <- rbind(all.simulated.predictions, prdC[[i]]$median.simulated.TTL, prdC[[i]]$individual.simulated.TTL)
}
colnames(all.simulated.predictions) <- c("drug", "sampleID", "centroid", "dose", "TTL.prediction")
```
...
for (centroid.val in unique(all.simulated.predictions$centroid)){
  ...
  for (c_k in 1:4){
    plot_delay_distribution(c_k, centroid.val, 38, 38.3)
  }
  dev.off()
}
```
