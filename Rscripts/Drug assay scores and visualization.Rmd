...
final.model.type <- "individual" #"individual" or "median"
...
Plotting the sigmoids of the drug on the chosen features versus the normal behavior of the OOL feature in order to see if the trends are actually opposites (or colinear) and that the chosen functions fit the data.
...
for (target_drug in unique(AllPenDoseresponse$drug)){
  ...
  for (target_feature in names(OOL_features)) {
    ...
    if (target_feature %in% AllPenDoseresponse$feature){
      plots[[paste0(target_feature,".median")]] <- plot_median_sigmoid(target_feature, target_drug)
    }else{
      plots[[paste0(target_feature,".median")]] <- ggplot() + theme_void()
    }
    ...
    if (target_feature %in% AllPenDoseresponse$feature){
      plots[[paste0(target_feature,".individual")]] <- plot_individual_sigmoids(target_feature, target_drug)
    }else{
      plots[[paste0(target_feature,".individual")]] <- ggplot() + theme_void()
    }
  }
  ...
  for (i in 0:floor(length(plots)/12)){
    grid.arrange(grobs = plots[(12*i+1):min(length(plots),(12*i+12))], nrow = 4, ncol = 3)
  }
  dev.off()
}
```
