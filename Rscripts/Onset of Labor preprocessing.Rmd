...
Here we set all the necessary path to import and save data and plots.

```{r}
current_dir <- getwd()
my_path <- sub("/[^/]+$", "", current_dir)
setwd(paste0(my_path,"/Rscripts"))
funct_path=paste0(my_path,"/Onset of Labor data/Onset of Labor Initial Data/Onset of Labor Study Reanalysis PTB drug assay matching gates.csv")
path_to_penalization_file=paste0(my_path,"/Onset of Labor data/Onset of Labor Initial Data/penalization matrix drug assay.xlsx")
out_path=paste0(my_path,"/Onset of Labor data/Preprocessed Data")
```

Creating a directory for the outcomes
...
MyData = MyData %>%
          ...
          dplyr::mutate(reagent = sub("^[^_]*_", "", reagent),
                        ID = paste0("P",ID, "_", GA)) %>% # Creating a new index ID
          dplyr::filter(ID != "Pinternalcontrol_na") %>%
          dplyr::select(-c(EGA, GA))
...
for(timepoint in unique(MyData$ID)){
  ...
  MyData_timepoint = MyData %>% dplyr::filter(ID == timepoint)
  ...
  MyData_timepoint = MyData_timepoint %>%
                        dplyr::mutate(feature = asinh(median/5)) %>%
                        dplyr::select(-c("DOS", "median")) %>%
...
TTL = read.csv(funct_path) %>%
        ...
        dplyr::filter(ID != "Pinternalcontrol_na") %>%
        dplyr::select(c(EGA, DOS, ID)) %>%
        dplyr::filter(!grepl("pp", ID)) %>%
        group_by(ID) %>%
        dplyr::filter(row_number()==1) %>%
        dplyr::mutate(DOS = as.numeric(DOS)) %>%
        dplyr::mutate(EGA = as.numeric(EGA))
```

Useful datasets to build ML models.
...
im_noEGA_DOS = merge(x, dplyr::select(TTL, "DOS"), by = "ID")
# Dataset with EGA
im_EGA = merge(x, dplyr::select(TTL, "EGA"), by = "ID")
# Dataset with EGA qnd DOS
im_EGA_DOS = merge(x, dplyr::select(TTL, c("EGA", "DOS")), by = "ID")
# Dataset containing the patient ID (for LeaveOneGroupOut cross validation)
Id <- dplyr::select(x, ID) %>% dplyr::mutate(patientID = as.factor(str_extract(ID, "(?<=P)\\d+")))
```
...
write.csv(im_EGA_DOS, paste0(out_path, "/immunome_EGA_DOS_OOL.csv"), row.names=FALSE)
write.csv(Id, paste0(out_path, "/patientID.csv"), row.names=FALSE)
```
...
EGA = subset(TTL, select = c(ID, EGA))
...
This is function taking a penalization matrix and the name of the stimulation associated with the matrix and returning the list of the 
...
```{r}
im_noEGA_pen <- dplyr::select(im_noEGA, -one_of(discard))
im_noEGA_DOS_pen <- dplyr::select(im_noEGA_DOS, -one_of(discard))
im_EGA_pen <- dplyr::select(im_EGA, -one_of(discard))
im_EGA_DOS_pen <- dplyr::select(im_EGA_DOS, -one_of(discard))
```
...
write.csv(im_EGA_DOS_pen, paste0(out_path, "/immunome_EGA_DOS_pen_OOL.csv"), row.names=FALSE)
```
