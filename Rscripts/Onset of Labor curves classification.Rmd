...
funct_path=paste0(my_path,"/Onset of Labor csv/immunome_EGA_DOS_pen_OOL.csv")
out_path=paste0(my_path,"/Onset of Labor csv")
```
...
Selecting if you want the following script to fit models on DOS or EGA and if you want to fit on term, preterm or all the data.
...
if (mode=="DOS"){
  ...
  save(df_models, file=paste(out_path, "/",cohort,"pen_model_curves_cytof.rda", sep=""))
}
if (mode=="EGA"){
...
# Saving the plots
pdf(paste(out_path, "/plots/best_features_plot_on_",mode,"_OOL.pdf", sep=""))
...
Plotting term and preterm features as functions of EGA on the same figures.

```{r}
#feature.index
load(paste0(OOL_path, "/feature_index.rda"))
# Looking at the features with index > 0
...
load(paste0(OOL_path, "/preterm_pen_model_curves_cytof_on_EGA.rda"))
df_preterm_models=df_models
df_preterm_sum=read.csv(file=paste0(OOL_path,"/preterm_pen_classification_curves_cytof_on_EGA.csv"))
...
load(paste0(OOL_path, "/term_pen_model_curves_cytof_on_EGA.rda"))
df_term_models=df_models
df_term_sum=read.csv(file=paste0(OOL_path,"/term_pen_classification_curves_cytof_on_EGA.csv"))
...
for (target_feature in feature_list) {
  ...
  # Plot
  plot <- ggplot() +
      geom_point(data=df_term_points, aes(x = x, y = y), color = "blue", size = 1, na.rm = TRUE) +
      geom_line(data=df_preds, aes(x = x, y = y_pred_term), color = "red") +
      geom_point(data=df_preterm_points, aes(x = x, y = y), color = "darkgreen", size = 1, na.rm = TRUE) +
      geom_line(data=df_preds, aes(x = x, y = y_pred_preterm), color = "orange") +
      ggtitle(paste(target_feature, "\n (red) term p-value = ",pval_term,"\n (orange) term p-value = ",pval_preterm, sep="")) +
...
# Saving the plots
pdf(paste(out_path, "/plots/best_features_plot_on_",mode,"term_vs_preterm_OOL.pdf", sep=""))
for (i in 0:floor(length(plots)/16)){
  grid.arrange(grobs = plots[(16*i+1):min(length(plots),(16*i+16))], nrow = 4, ncol = 4)
}
dev.off()
