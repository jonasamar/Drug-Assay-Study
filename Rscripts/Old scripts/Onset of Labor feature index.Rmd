...
#CYTOF <- dplyr::select(data, -EGA) #Removing EGA (or not)
# Preprocessing
...
load(paste0(OOL_path, "/Pvalue threshold optimization with EGA.rda"))
...
prdC=foreach(i=seq(npt),.packages = c("dplyr", "tibble")) %dopar% xxx(data.matrix(CYTOF), Yh, Id, i, parm, FALSE) #data.matrix(EGA)
list_preds<-aggregation.of.predictions(prdC)
# Closing backend configuration
stopCluster(cl)
```
...
for (i in seq(1, length(cols))){
  ...
  coefs <- c()
  for (iter in seq(prdC)){
    coefs[iter] <- abs(prdC[[iter]]$EN_coef[i])
  }
